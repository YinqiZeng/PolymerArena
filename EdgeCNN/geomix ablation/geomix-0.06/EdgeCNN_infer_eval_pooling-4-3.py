...
parser.add_argument("--aug_ratio", type=float, default=0.06, help='number of mixup pairs')
parser.add_argument("--num_graphs", type=int, default=5, help='number of mixup graphs per pair')
parser.add_argument("--num_nodes", type=int, default=10, help='number of nodes in the mixup graph')
...
# -----------------------------------------------------------------
def normalize_dataset_keys(dataset, keys=("edge_attr",)):
    """
    Ensure every Data object in the dataset has the same attribute keys.
    """
    for d in dataset:
        for k in keys:
            if not hasattr(d, k):
...
def smiles_to_graph(smiles: str):
    try:
        ...
        data.edge_weight = torch.ones(E, dtype=torch.float)
        return data
    except Exception:
        print("wrong SMILES")
        return None
...
print({tuple(sorted(d._store.keys())) for d in dataset})
...
print({tuple(sorted(d._store.keys())) for d in train_dataset})
...
# ----------------------------
num_epochs = 500
log_path = f"loss_log_{POOLING_TYPE}_512.csv"
...
torch.save({
    ...
    f"multitask_edgeCNN_last_{POOLING_TYPE}_512.pth"
)
...
torch.save({
    ...
    f"multitask_edgeCNN_best_{POOLING_TYPE}_512.pth"
)
...
checkpoint = torch.load(f'multitask_edgeCNN_best_{POOLING_TYPE}_512.pth', weights_only=False)
...
output_csv = f"R2&RMSE_edgeCNN_{POOLING_TYPE}_512.csv"
...
for i, prop in enumerate(REG_COLS + [BIN_COL]):  # Includes both regression and arom_logit
    ...
    plt.savefig(f"{prop}_pearsonr_edgeCNN_{POOLING_TYPE}_512_plot.pdf", dpi=600, bbox_inches="tight")
